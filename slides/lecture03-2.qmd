...
$$
\begin{gather*}
y_t = \Lambda(F_t; \theta) + \zeta_t + \varepsilon_t \\
...
The likelihood is $\zeta + \varepsilon \sim N(0, \Sigma)$.

$$
\begin{gather*}
\Sigma = V + D \\
D = \text{diag}(\omega_t^2)
V = \frac{\sigma^2}{1-\rho^2}\begin{pmatrix}1 & \rho \ldots & \rho^{T-1} \\ \rho & 1 & \ldots & \rho^{T-2} \\ \rho^2 & \rho \ldots & \rho^{T-3} \\ \vdots & \vdots & \ddots & \vdots \\ \rho^{T-1} & \rho^{T-2} & \ldots & 1\end{pmatrix}
\end{gather*}
$$
...
Updating the prior with the likelihood of new data.
...
lprior(μ, σ) = logpdf(Normal(0, 1), μ) + logpdf(truncated(Normal(0, 1), 0, Inf), σ)
...
p2 = plot(return_periods, return_levels, linewidth=3, color=:blue, label="Model Fit", tickfontsize=16, legendfontsize=18, guidefontsize=18, bottom_margin=5mm, left_margin=5mm, legend=:bottomright)
...
- **Prior is part of the model! Can evaluate and change based on predictive checks**.
