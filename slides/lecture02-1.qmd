...
We often work with **log-likelihoods**.

::: {.fragment .fade-in}
**There is a lot to maximum likelihood estimation which we will only touch on.**
:::
...
## Probabilistic Interpretation of RMSE Minimization

**Claim**: Minimizing the (R)MSE is the same as maximizing likelihood assuming normally-distributed residuals (with known variance).

$$
\begin{gather*}
\mathbf{y} = F(\mathbf{x}; \theta) - \mathbf{r} \\
r_i \sim \mathcal{N}(0, \sigma)
\end{gather*}
$$
...
## Simplifying by Ignoring Constants
