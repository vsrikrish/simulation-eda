...
For this type of null hypothesis test, our **test statistic** is the slope of the OLS fit $$\hatab} = \frac{\sum_{i=1}^n (x_i - \bar{x})(y_i - \bar{y})}{(x_i - \bar{x})^2}.$$

::: {.fragment .fade-in}
After some algebra, **assuming the null** $a = 0$, the **sampling distribution** of the statistic is $$\frac{\hat{a}}{SE_{\hat{a}}} \sim t_{n-2}.$$
:::
...
If the p-value is sufficiently small (below $\alpha$), we say that we can **reject the null hypothesis** with $1-\alpha$ confidence, or that **the alternative hypothesis is statistically significant** at the $1-\alpha$ level.
...
:::: {.columns}
::: {.column width=50%}
1. Probability that the null hypothesis is true (or by "chance");
2. An indication of the effect size.
:::
::: {.column width=50%}
...
If we have a **generative** model which permits simulation of data:
