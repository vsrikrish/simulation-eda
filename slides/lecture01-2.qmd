...
## Confidence Intervals: Example (95%)
...
ci_frac1 = 100 * (sum((mc_ci[:, 1] .< mean_true) .&& (mc_ci[:, 2] .> mean_true))) ./ n_cis

# plot CIs
p1 = plot([mc_ci[1, :]], [1, 1], linewidth=3, color=:blue, label="95% Confidence Interval", title="100 Samples with Size 100", yticks=:false, tickfontsize=14, titlefontsize=20, legend=:false, guidefontsize=16)
for i = 2:n_cis
    plot!(p1, [mc_ci[i, :]], [i, i], linewidth=2, color=:blue, label=:false)
end
vline!(p1, [mean_true], color=:red, linewidth=2, linestyle=:dash, label="True Value") # plot true value as a vertical line
xaxis!(p1, "Estimate")
...
# compute percentage of CIs which contain the true value
ci_frac2 = 100 * (sum((mc_ci[:, 1] .< mean_true) .&& (mc_ci[:, 2] .> mean_true))) ./ n_cis

# plot CIs
p2 = plot([mc_ci[1, :]], [1, 1], linewidth=3, color=:blue, label="95% Confidence Interval", title="100 Samples with Size 1,000", yticks=:false, tickfontsize=14, titlefontsize=20, legend=:false, guidefontsize=16)
for i = 2:n_cis
    plot!(p2, [mc_ci[i, :]], [i, i], linewidth=2, color=:blue, label=:false)
end
vline!(p2, [mean_true], color=:red, linewidth=2, linestyle=:dash, label="True Value") # plot true value as a vertical line
xaxis!(p2, "Estimate")
