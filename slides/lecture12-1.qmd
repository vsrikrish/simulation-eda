...
Generally based on the error (RMSE, MAE) or probability of the data $p(y | \mathbf{\mathcal{M}})$.

To select a model:

$$\argmax_{M_i \in \mathcal{M}} p(y |M_i)$$
...
2. Time nonstationary model, $y_t \sim \text{GEV}(\mu_0 + \mu_1 t, \sigma, \xi);$
...
$$\max_i p(y | \hat{\theta}, \mathcal{M}_i)$$

or the posterior $\Theta$ (if this was found):

$$\max_i \int_{\theta \in \Theta} p(y | \theta, M)$$
