...
- Estimate $$\mu = \mathbb{E}_p[f(x)] \approx \frac{1}{N} \sum_{i=1}^n f(x_n) = \hat{\mu}_n$$

## Monte Carlo Error Analysis

- Standard error $$\hat{\sigma}_n = \frac{\sigma_{f(x)}}{\sqrt{n} \approx \frac{\hat{\sigma}_{f(x)|_{i=1}^n}}{\sqrt{n-1}}$$
...
p1 = histogram(u, ylabel="count", xlabel=L"$u$ (m/s)", label=false, tickfontsize=16, guidefontsize=18, size=(450, 450))
p2 = histogram(Cin, ylabel="count", xlabel=L"$C_{in}$ (ppm)", label=false, tickfontsize=16, guidefontsize=18, size=(450, 450))
p3 = histogram(R, ylabel="count", xlabel=L"$R$ (ppm/s)", label=false, tickfontsize=16, guidefontsize=18, size=(450, 450))
display(p1)
display(p2)
display(p3)
```
...
$$\hat{\sigma}_n = \sqrt{\frac{\text{Var}(\mathbb{I}[x_{1:n} > 0.14])}{n-1}}$$
:::
::: {.column width=50%}
```{julia}
#| echo: true
...
for i = 2:nsamp
    ...
    std_mc_out[i] = 1/sqrt(i-1) * std(mean_SO2[1:i] .> 0.14)
end
